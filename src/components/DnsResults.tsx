...
export const DnsResults: React.FC<{ results: DomainResult[] }> = ({ results }) => {
  ...
  const renderDomainCard = (result: DomainResult, children: React.ReactNode) => (
    <Card key={result.id} className="overflow-hidden mb-4">
  ...
  const renderContentForTab = (result: DomainResult, tab: 'SPF' | 'DMARC' | 'BIMI') => {
    // This function will now only render the content, not the whole card
    switch (tab) {
        case 'SPF':
            return (
              <div>
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="spf" className="w-full">
        ...
        <TabsContent value="spf" className="mt-4">
            {results.filter(r => r.lookupType === 'ALL' || r.lookupType === 'SPF').map(result => renderDomainCard(result, renderContentForTab(result, 'SPF')))}
        </TabsContent>
        <TabsContent value="dmarc" className="mt-4">
            {results.filter(r => r.lookupType === 'ALL' || r.lookupType === 'DMARC').map(result => renderDomainCard(result, renderContentForTab(result, 'DMARC')))}
        </TabsContent>
        <TabsContent value="bimi" className="mt-4">
            {results.filter(r => r.lookupType === 'ALL' || r.lookupType === 'BIMI').map(result => renderDomainCard(result, renderContentForTab(result, 'BIMI')))}
        </TabsContent>
      </Tabs>
    </div>
  );
